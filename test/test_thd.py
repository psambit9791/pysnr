...
class TestTHD(unittest.TestCase):
    ...
    def test_thd_signal(self):
        ...
        self.assertTrue(np.isclose(pysnr.thd_signal(signal + noise, Fs), -86.1283, rtol=0.025))

        Fi, Fs, N, noise, signal = self.get_signal_data(self.cosine)
        self.assertTrue(np.isclose(pysnr.thd_signal(signal + noise, Fs), -86.1291, rtol=0.025))

    def test_thd_psd(self):

        Fi, Fs, N, noise, signal = self.get_signal_data(self.sine)
        f, pxx = pysnr.periodogram(signal+noise, Fs, window=('kaiser', 38))
        self.assertTrue(np.isclose(pysnr.thd_power_spectral_density(pxx, f), -86.1283, rtol=0.025))

        Fi, Fs, N, noise, signal = self.get_signal_data(self.cosine)
        f, pxx = pysnr.periodogram(signal+noise, Fs, window=('kaiser', 38))
        self.assertTrue(np.isclose(pysnr.thd_power_spectral_density(pxx, f), -86.1291, rtol=0.025))
    ...
    def test_thd_power(self):
        ...
        self.assertTrue(np.isclose(pysnr.thd_power_spectrum(sxx, f, rbw), -86.1283, rtol=0.025))

        Fi, Fs, N, noise, signal = self.get_signal_data(self.cosine)
        f, sxx = pysnr.periodogram(signal+noise, Fs, window=('kaiser', 38), scaling="spectrum")
        w = scipy.signal.windows.kaiser(len(signal), 38)
        rbw = pysnr.utils.enbw(w, Fs)
        self.assertTrue(np.isclose(pysnr.thd_power_spectrum(sxx, f, rbw), -86.1291, rtol=0.025))

    def test_thd_aliased(self):
        Fi, Fs, N, noise, signal = self.get_signal_data(self.aliased)
        self.assertTrue(np.isclose(pysnr.thd_signal(signal + noise, Fs, aliased=False), -29.1114, rtol=0.025))
        self.assertTrue(np.isclose(pysnr.thd_signal(signal + noise, Fs, aliased=True), -22.5413, rtol=0.025))
